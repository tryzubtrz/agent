...
def _is_configured(alias: str, env_name: str, allow_runtime_vault: bool) -> tuple[bool, str]:
    if os.getenv(env_name, "").strip(): return True, "server-environment"
    if allow_runtime_vault and secret_configured(alias): return True, "encrypted-runtime-vault"
    return False, "not-configured"


@router.get("/api/vault/bootstrap/public-key")
def vault_bootstrap_public_key() -> dict[str, object]:
    try: public_key = bootstrap_public_key()
    except VaultUnavailableError as exc: raise HTTPException(status_code=503, detail="Runtime vault bootstrap key is unavailable.") from exc
...
def list_vault_aliases(_principal: Principal = Depends(require_owner)) -> list[dict[str, object]]:
    ...
    for alias, (env_name, purpose, allow_runtime_vault) in _FIXED_ALIASES.items():
        ...
        result.append({"alias": alias, "purpose": purpose, "configured": configured, "source": source, "value_exposed": False})
    return result
...
def vault_health(_principal: Principal = Depends(require_owner)) -> dict[str, object]:
    ...
    return {"status": "ok", "aliases": len(aliases), "configured": sum(1 for item in aliases if item["configured"]), "raw_secret_exposure": False, "encrypted_runtime_storage": True, "bootstrap_transport": "sealed-public-key-only"}
